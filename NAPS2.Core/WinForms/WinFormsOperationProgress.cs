...
namespace NAPS2.WinForms
{
    public class WinFormsOperationProgress : IOperationProgress
    {
        ...
        public void ShowBackgroundProgress(IOperation op)
        {
            ...
            if (!op.IsFinished)
            {
                // Check if a handle is created and can use the invoke, else call the function directly and create the handle
               if (notificationManager.ParentForm.IsHandleCreated)
                    notificationManager.ParentForm.BeginInvoke(() => notificationManager.OperationProgress(this, op));
                else
                {
                    notificationManager.ParentForm.CreateControl();
