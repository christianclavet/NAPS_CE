...
namespace NAPS2.WinForms
{
    public partial class FDesktop : FormBase
    {
        ...
        private string title = System.Windows.Forms.Application.ProductName.ToString() + " " + System.Windows.Forms.Application.ProductVersion.ToString();
        ...
        public FDesktop(ImageSettingsContainer imageSettingsContainer, StringWrapper stringWrapper, AppConfigManager appConfigManager, RecoveryManager recoveryManager, OcrManager ocrManager, IProfileManager profileManager, IScanPerformer scanPerformer, IScannedImagePrinter scannedImagePrinter, ChangeTracker changeTracker, StillImage stillImage, IOperationFactory operationFactory, IUserConfigManager userConfigManager, KeyboardShortcutManager ksm, ThumbnailRenderer thumbnailRenderer, WinFormsExportHelper exportHelper, ScannedImageRenderer scannedImageRenderer, NotificationManager notify, CultureInitializer cultureInitializer, IWorkerServiceFactory workerServiceFactory, IOperationProgress operationProgress, UpdateChecker updateChecker, ScannedImageHelper scannedImageHelper)
        {
            ...
            title = System.Windows.Forms.Application.ProductName.ToString() + " " + System.Windows.Forms.Application.ProductVersion.ToString();
            if (Text != null)
        ...
        private async void FDesktop_Shown(object sender, EventArgs e)
        {
            ...
            Pipes.StartServer(msg =>
            {
                ...
                if (msg.Equals(Pipes.MSG_ACTIVATE))
                {
                    SafeInvoke(() =>
                    {
                        var form = System.Windows.Forms.Application.OpenForms.Cast<Form>().Last();
        ...
        private async Task ScanWithDevice(string deviceID)
        {
            ...
            if (profile != null)
            {
                ...
                await scanPerformer.PerformScan(profile, new ScanParams(), this, ReceiveScannedImage());
                Activate();
            }
        }
        ...
        public async Task ScanDefault()
        {
            ...
            {
                await scanPerformer.PerformScan(profileManager.DefaultProfile, param, this, ReceiveScannedImage());
        ...
        private async Task ScanWithNewProfile()
        {
            ...
            await scanPerformer.PerformScan(editSettingsForm.ScanProfile, new ScanParams(), this, ReceiveScannedImage());
        ...
        public void setPreviewImage(Bitmap image, bool frontpage = true)
        {
            if (frontpage)
            {
        ...
        //Used to test stuff. (Trigger via the keyboard letter Q)
        private async void Test() 
        {
            // Test if I can make the OCR of all the images
            statusStrip1.Items[0].Text = "OCR starting";
            ScanParams param = new ScanParams();
            for (int i = 0; i < imageList.Images.Count; i++)
            {
                Bitmap result = await scannedImageRenderer.Render(imageList.Images[i]);
                string tempPath = scannedImageHelper.SaveForBackgroundOcr(result, param, true);
                scannedImageHelper.RunBackgroundOcr(imageList.Images[i], param, tempPath, true);
                statusStrip1.Items[0].Text = "OCR doing page "+i.ToString()+" in "+ imageList.Images.Count.ToString();
            }
            statusStrip1.Items[0].Text = "OCR completed!";
            MessageBox.Show("Test Completed!", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void DisplaySelectedItem_info(bool minimum = false)
        {
            ...
            if (SelectedIndices.Count() == 1)
            {
                ...
                if (imageList.Images.Count-1 < SelectedIndices.First())
                    return;

                if (imageList.Images.Count-1 < thumbnailList1.SelectedItems[0].Index)
                {
                    statusStrip1.Items[0].Text = MiscResources.No_selection;
                    UpdateToolbar();
                    return;
                ...
                String text = ((SelectedIndices.First()) + 1).ToString();
                String text2 = imageList.Images[SelectedIndices.First()].infoResolution;
                
                String text3 = imageList.Images[SelectedIndices.First()].BarCodeData;
                
                String text4 = "";
                if (text3 != "" && text3 != null)
                    text4 = "Barcode: " + text3;
                String format = imageList.Images[SelectedIndices.First()].infoFormat;
                string side;
                if (imageList.Images[SelectedIndices.First()].SheetSide == 1)
                    side = "- side: front";
                else if (imageList.Images[SelectedIndices.First()].SheetSide == 2)
                    side = "- side: back";
                else
                ...
                if (!minimum)
                {
                    var img = imageList.Images[SelectedIndices.First()];
        ...
        private void tsImport_Click_1(object sender, EventArgs e)
       {
        ...
        private void contextMenuStrip_Opening(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            if (!SelectedIndices.Any())
            {
               ctxScanBarCode.Enabled = false;
            }
            else
            {
                ctxScanBarCode.Enabled = true;
            }

            if (Clipboard.ContainsImage() || (Clipboard.GetDataObject()!=null))
            {
        ...
        private async void CopyImages()
        {
            if (SelectedIndices.Any())
            {
                ...
                //this should copy the image to the clipboard
                Bitmap bit = await scannedImageRenderer.Render(SelectedImages.First());
                bitmap = new Bitmap(bit);
                Clipboard.SetImage((Bitmap)bit);
                bit.Dispose();
            }
        }
        ...
        private void PasteImages()
        {
            ...
            // Check if the clipboard contains a bitmap but no data
            if (Clipboard.ContainsImage())
            {
                ScannedImage img1 = new ScannedImage((Bitmap)Clipboard.GetImage(),ScanBitDepth.C24Bit,true,100);
                imageList.Images.Add(img1);
                img1.ThumbnailChanged += ImageThumbnailChanged;
            ...
            // Check if the clipboard does not contains a bitmap or any data
            if (!CanPaste)
            {
                return;
            }

            // Check if the clipboard contains a DirectImageTransfer object
        ...
        private bool CanPaste
        {
            ...
            {
                var ido = Clipboard.GetDataObject();
                return ido != null && ido.GetDataPresent(typeof(DirectImageTransfer).FullName) && Clipboard.ContainsImage();
            }
        }
        ...
        private static bool AppendRtfEncodedImage(System.Drawing.Image image, ImageFormat format, StringBuilder sb, bool par)
        {
        ...
        private void thumbnailList1_DragDrop(object sender, DragEventArgs e)
        {
            ...
            else if (e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                if (SelectedIndices.Any())
                {
                    var box = MessageBox.Show("Écraser l'image sélectionnée", "Importation", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
        ...
        private void closeWorkspace(bool clear=true)
        {
            ...
            else if (!clear)
            {
                ...
                var result1 = MessageBox.Show("Changes have been saved!", MiscResources.UnsavedChanges ,
