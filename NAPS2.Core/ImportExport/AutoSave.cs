...
namespace NAPS2.ImportExport
{
    public class AutoSave : IAutoSave
    {
        ...
        {
            this.notify = FDesktop.GetInstance().GetNotificationManager();
            ...
            try
            {
                ...
                {
                    (bool success, string filePath) = await SaveOneFile(settings, now, i++, imageList);
                    if (!success)
                    {
                        ok = false;
                    }
        ...
        private async Task<(bool, string)> SaveOneFile(AutoSaveSettings settings, DateTime now, int i, List<ScannedImage> images)
        {
            this.notify = FDesktop.GetInstance().GetNotificationManager();
