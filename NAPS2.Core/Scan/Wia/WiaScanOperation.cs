...
namespace NAPS2.Scan.Wia
{
    public class WiaScanOperation : OperationBase
    {
        ...
        private void ConfigureProps(WiaDevice device, WiaItem item)
        {
            ...
            //Try to set pages properties forcing at 1 when rescanning a page
            if (ScanParams.RescanMode)     
                item.SetProperty(WiaPropertyId.DPS_PAGES, 1);
            else
                item.SetProperty(WiaPropertyId.DPS_PAGES, 0);
            ...
            if (ScanProfile.AutoBorderDetection) //Try to auto scale the document. Work on Kodak Scannners.
                item.SetProperty(WiaPropertyId.IPS_PAGES_SIZE, 100); 

            if (ScanProfile.AutoDeskew)  //Auto deskew
                item.SetProperty(WiaPropertyId.IPS_AUTO_DESKEW, WiaPropertyValue.AUTO_DESKEW_ON);
            else
                item.SetProperty(WiaPropertyId.IPS_AUTO_DESKEW, WiaPropertyValue.AUTO_DESKEW_OFF);

            if (ScanProfile.DoubleFeedAction == 1) // Multi-Feed detection for stopping when a double is detected. //Not working at the moment.
                item.SetProperty(WiaPropertyId.IPS_MULTI_FEED, WiaPropertyValue.MULTI_FEED_DETECT_STOP_SUCCESS);
            else 
                item.SetProperty(WiaPropertyId.IPS_MULTI_FEED, WiaPropertyValue.MULTI_FEED_DETECT_DISABLED);
